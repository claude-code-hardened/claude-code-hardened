...
用 `@claude-code-hardened/workflow-engine` **直接**运行一个 workflow，绕开 Workflow 工具与核心 `runAgent`。
...
- **库可独立使用**：`run.ts` 只 `import { runWorkflow, ... } from '@claude-code-hardened/workflow-engine'`，自己组装 7 个端口，不依赖 `src/` 任何核心模块。
