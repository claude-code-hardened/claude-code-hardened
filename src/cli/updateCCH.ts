/**
 * `cch update` — Check and install the latest version of claude-code-hardened.
 *
 * Detection strategy:
...
const PACKAGE_NAME = 'claude-code-hardened'
...
export async function updateCCH(): Promise<void> {
  ...
  if (latestVersion === currentVersion || gte(currentVersion, latestVersion)) {
    writeToStdout(chalk.green(`cch is up to date (${currentVersion})`) + '\n')
