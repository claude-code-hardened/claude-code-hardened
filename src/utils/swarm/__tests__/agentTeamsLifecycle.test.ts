...
describe('Agent Teams lifecycle', () => {
  test('runs TeamCreate -> spawn -> TaskUpdate -> SendMessage -> TeamDelete', async () => {
    const { TeamCreateTool } = await import(
      '@claude-code-hardened/builtin-tools/tools/TeamCreateTool/TeamCreateTool.js'
    )
    const { spawnTeammate } = await import(
      '@claude-code-hardened/builtin-tools/tools/shared/spawnMultiAgent.js'
    )
    const { TaskCreateTool } = await import(
      '@claude-code-hardened/builtin-tools/tools/TaskCreateTool/TaskCreateTool.js'
    )
    const { TaskUpdateTool } = await import(
      '@claude-code-hardened/builtin-tools/tools/TaskUpdateTool/TaskUpdateTool.js'
    )
    const { SendMessageTool } = await import(
      '@claude-code-hardened/builtin-tools/tools/SendMessageTool/SendMessageTool.js'
    )
    const { TeamDeleteTool } = await import(
      '@claude-code-hardened/builtin-tools/tools/TeamDeleteTool/TeamDeleteTool.js'
    )
