...
import {
  type AgentColorName,
  setAgentColor,
} from '@claude-code-hardened/builtin-tools/tools/AgentTool/agentColorManager.js';
import {
...
export function AgentEditor({ agent, tools, onSaved, onBack }: Props): React.ReactNode {
  ...
  const handleSave = useCallback(
    async (changes: SaveChanges = {}) => {
      ...
      try {
        ...
        onSaved(t('Updated agent: {{n}}', { n: chalk.bold(agent.agentType) }));
        return true;
      } catch (err) {
  ...
  const renderMenu = (): React.ReactNode => (
    <Box flexDirection="column" tabIndex={0} autoFocus onKeyDown={handleMenuKeyDown}>
      <Text dimColor>{t('Source: {{v}}', { v: getAgentSourceDisplayName(agent.source) })}</Text>
